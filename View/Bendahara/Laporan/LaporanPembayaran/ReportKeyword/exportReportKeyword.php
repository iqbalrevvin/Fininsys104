...
$result = $db->query("SELECT * FROM siswa JOIN kelas ON kelas.nama_kelas = siswa.kelas
                                    ...
                                    AND master_transaksi.nama_master_transaksi LIKE '%$keyword%'
                                    OR jenis_transaksi.nama_jenis_transaksi LIKE '%$keyword%' 
									ORDER BY tgl_transaksi ASC ");

//pengaturan nama file
$date = date('d-m-Y');
$namaFile = "ReportMasterTransaction-$date.xls";
//pengaturan judul data
$judul = "Data Laporan Berdasarkan Kata Kunci | Jika Tampil -PROTECTED VIEW- Lakukan Pengaturan pada POLICY SETTINGS di Aplikasi Excel Anda Untuk Dapat Mengelola Data!";
