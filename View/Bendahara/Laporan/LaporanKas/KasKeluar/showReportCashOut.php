<?php 
	include"Controller/Bendahara/Laporan/LaporanKas/KasKeluar/reportCashOutQuery.php";
	include "Controller/Bendahara/PengaturanData/DataSekolah/profilSekolahQuery.php";

	if(@$_POST['JnsKasKeluar'] == 'semua'){
 		$totalKasKeluar=$db->query("select SUM(jml_kas_keluar) AS jumlah FROM kas WHERE tgl_kas BETWEEN '$FromDate' AND '$EndDate' AND tipe_kas='KELUAR'");
 	}else{
 		$totalKasKeluar=$db->query("select SUM(jml_kas_keluar) AS jumlah FROM kas WHERE tgl_kas BETWEEN '$FromDate' AND '$EndDate' AND tipe_kas='KELUAR' AND idJenis_kas = '$JenisKas' ");
 	}
    $total= $totalKasKeluar->fetch_object();
    $jam = date('H:i:s');
?>
<div class="header">
	<h2 style="text-align: center;">
    	...
    	<a href="View/Bendahara/Laporan/LaporanKas/KasKeluar/exportReportCashOut.php?FD=<?php echo $FromDate ?>&ED=<?php echo $EndDate ?>&JNS=<?php echo $JenisKas ?>">
...
<div id="invoice">
	<h4 style="text-align: center;">
		<span class="font-bold font-14">LAPORAN TRANSAKSI KAS KELUAR</span><br>
	...
	<table style="height: 0px; margin-left: 0; margin-right: auto; width: 400px;">
		<tbody class="font-12">
			...
			<tr>
				<td>Jenis Kas Keluar</td>
				<td>:</td>
				<?php if(@$_POST['JnsKasKeluar'] == 'semua'){ ?>
					<td>Semua Jenis</td>
				<?php }else{ ?> 
	...
	<table class="table table-bordered table-striped table-hover" 
		...
		<tbody>
			...
			<tr style="height: 10px;">
				...
				<td style="width: 0px; height: 10px; text-align: center; vertical-align: middle;">
					<b>Rp.<?= number_format($report->jml_kas_keluar) ?>.-</b>
				</td>
			</tr>
			<?php endwhile; ?>
		</tbody>
		...
		<tbody>
			<tr>
				...
				<td style="width: 171px;"><p><span style="font-size: 10px;">
					...
					<p>&nbsp;</p>
					<p>&nbsp;</p>
					<h6><p><b><span style="text-decoration: underline;"> <?= $session['nama_tampilan'] ?></span></b></p></h6>
				</td>
			</tr>
		</tbody>
	</table>
