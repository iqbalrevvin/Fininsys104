...
$result = $db->query("SELECT * FROM kas JOIN jenis_kas ON kas.idJenis_kas = jenis_kas.idJenis_kas
                                    WHERE kas.tgl_kas BETWEEN '$FromDate' AND '$EndDate' AND kas.tipe_kas = 'KELUAR' AND kas.idJenis_kas = '$Jenis'
                                    ORDER BY kas.tgl_kas ASC") or die ($db->error);
}

//pengaturan nama file
$date = date('d-m-Y');
$namaFile = "ReportCashOut-$date.xls";
//pengaturan judul data
$judul = "Data Laporan Kas Keluar";
$judul2 = "123!";
...
xlsWriteNumber($tablebody,$kolombody++,$data['jml_kas_keluar']);
