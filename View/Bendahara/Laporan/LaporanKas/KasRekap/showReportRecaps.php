<?php 
	include"Controller/Bendahara/Laporan/LaporanKas/KasRekap/reportRecapsQuery.php";
	include "Controller/Bendahara/PengaturanData/DataSekolah/profilSekolahQuery.php";

	//IDENTIFIKASI KAS MASUK
	$totalKasMasuk=$db->query("select SUM(jml_kas_masuk) AS total FROM kas WHERE tgl_kas BETWEEN '$FromDate' AND '$EndDate' AND tipe_kas='MASUK' ");
    $jmlKasMsk= $totalKasMasuk->fetch_object();
    //-------------------------------------------------------------------------------------------------------------------------------
    //IDENTIFIKASI KAS KELUAR
 	$totalKasKeluar=$db->query("select SUM(jml_kas_keluar) AS total FROM kas WHERE tgl_kas BETWEEN '$FromDate' AND '$EndDate' AND tipe_kas='KELUAR' ");
...
<div class="header">
	<h2 style="text-align: center;">
    	...
    	<a href="View/Bendahara/Laporan/LaporanKas/KasRekap/exportReportCashRecaps.php?FD=<?php echo $FromDate ?>&ED=<?php echo $EndDate ?>">
...
<div id="invoice">
	<h4 style="text-align: center;">
		<span class="font-bold font-14">LAPORAN REKAPITULASI KAS</span><br>
	...
	<table class="table table-bordered table-striped table-hover" 
		style="width: 1200; height: 0px; margin-left: auto; margin-right: auto;" border="2">
		<thead style="background-color: #607D8B;">
			<tr style="height: 0px;">
				<th style="width: 0px; text-align: center; height: 0px;">No.</th>
				<th style="width: 0px; text-align: center; height: 0px;">Kode Kas</th>
				<th style="width: 0px; text-align: center; height: 0px;">Jenis Kas</th>
				<th style="width: 0px; text-align: center; height: 0px;">Deskripsi</th>
				<!--<th style="width: 0px; text-align: center; height: 0px;">Petugas</th>-->
				<th style="width: 100px; text-align: center; height: 0px;">Tanggal</th>
				<th style="width: 0px; text-align: center; height: 0px;">Debit</th>
				<th style="width: 0px; text-align: center; height: 0px;">Kredit</th>
				<th style="width: 0px; text-align: center; height: 0px;">Saldo</th>
			</tr>
		</thead>
		<tfoot>
			<tr style="height: 0px;">
		...
		<tbody class="font-bold font-12">
				<?php
				$no = 1;
				while($report=$reportCashOutQuery->fetch_object()):
					error_reporting(0);
				...
				?>
			<tr style="height: 0px;">
				<td style="width: 0px; height: 0px; text-align: center; vertical-align: middle;">
					<?= sprintf('%05d', $no++) ?>
				</td>
				<td style="width: 0px; height: 0px; text-align: center; vertical-align: middle;">
					<strong><?= $report->no_kas ?></strong>
				</td>
				<td style="width: 0px; height: 0px; text-align: left; vertical-align: middle;">
				...
				<td style="width: 100px; height: 0px; text-align: center; vertical-align: middle;">
