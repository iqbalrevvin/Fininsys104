...
$jmlHrIniMsk 	= $db->query("SELECT sum(jml_kas_masuk) AS jumlah FROM kas_khusus WHERE day(tgl_kas) = '$day' AND tipe_kas = 'MASUK' 
							  AND idMaster_kas = '$ID'") or die ($db->error);
$hrIniMsk 		= $jmlHrIniMsk->fetch_object();
#-----------------------------------------------------------------------------------------------
#JUMLAH UANG MASUK BULAN INI
$jmlBlnIniMsk = $db->query("SELECT sum(jml_kas_masuk) AS jumlah FROM kas_khusus WHERE month(tgl_kas) = '$now' AND tipe_kas = 'MASUK' AND idMaster_kas = '$ID' ") or die ($db->error);
$blnIniMsk = $jmlBlnIniMsk->fetch_object();
#-----------------------------------------------------------------------------------------------
#JUMLAH UANG KELUAR HARI INI
$jmlHrIniKlr = $db->query("SELECT sum(jml_kas_keluar) AS jumlah FROM kas_khusus WHERE day(tgl_kas) = '$day' AND tipe_kas = 'KELUAR' 
							AND idMaster_kas = '$ID' ") or die ($db->error);
$hrIniKlr = $jmlHrIniKlr->fetch_object();
#-----------------------------------------------------------------------------------------------
#JUMLAH UANG KELUAR BULAN INI
$jmlBlnIniKlr = $db->query("SELECT sum(jml_kas_keluar) AS jumlah FROM kas_khusus WHERE month(tgl_kas) = '$now' AND tipe_kas = 'KELUAR'
							AND idMaster_kas = '$ID' ") or die ($db->error);	
